...
# 평균 성적이 70점 이상인 사용자의 이름과 나이를 출력
def get_filter_user(users):
    avg = []
    for i in range(len(users)):
        s = (users[i]["math_score"] + users[i]["science_score"] + users[i]["english_score"] + users[i]["social_score"])//4
        
        if s >= 70:
            avg.append({'age': users[i]["age"], 'name': users[i]["name"]})
    return(avg)
